...
1. **Settings → Devices & Services → Add Integration → Blaueis Midea AC.** The first time, Home Assistant installs the Python packages `blaueis-core` and `blaueis-client` from PyPI before the dialog opens — the HA host needs internet access for this, and it can take a minute.
